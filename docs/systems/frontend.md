...
- `components/Modals/` — `Modal` shell + `UserQuestionModal` (BG3-style bottom-center translucent question dialog: radio options + free-text choice, submits via `sendMessage`; opens from the "You have a question" banner or by clicking any `user-question` chat bubble, keyboard Enter/Space, inner text-expand toggles/links excluded) + `StartupSetup`, `NewProject`, `ProjectSettings`, `SpawnAgent`, `AgentEdit`, `McpManagement`, `KnowledgeBase`, `MarshallAudit`.
