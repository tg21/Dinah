...
- `components/Drawer/` — `Drawer` always mounted (collapse is a CSS slide so chat/tab state survives) + `ChatTab` (autogrow textarea, raw-text JSX rendering — never pre-escape, React escapes itself), `StatsTab`, `ThoughtsTab` (rendered as the BTS tab: behind-the-scenes reasoning + `QUEUE_*` delivery traces, auto-scrolls to latest), `ContextTab`, `BrainView` (three.js), `NetworkTab` (rendered as the A-to-A tab: agent-to-agent traffic only), Messages tab (see `messaging.md`).
- `components/Modals/` — `Modal` shell + `UserQuestionModal` (BG3-style bottom-center translucent question dialog: radio options + free-text choice, submits via `sendMessage` with the `questionId`; opens from the "You have a question" banner — which counts open holds — or by clicking any unanswered `user-question` chat bubble, keyboard Enter/Space, inner text-expand toggles/links excluded; option labels wrap, detail box auto-grows) + `StartupSetup`, `NewProject`, `ProjectSettings`, `SpawnAgent`, `AgentEdit`, `McpManagement`, `KnowledgeBase`, `MarshallAudit`.
