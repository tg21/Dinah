...
- **Staffing visibility (generic, no per-pair helpers)**. All agent→agent exchanges go through the durable queue (`enqueueDirectMessage` in `messageQueueService.js`), which emits `courier_message`, writes the shared log, and shows in message-activity. Chat drawers stay user-only: the queue never projects into `agent-*.msgs.json` (no inbox/sent copies, no `[claimed]`/`[completed]` stage markers). `requestAgentSummoning` sends requester→HR, HR confirm/spawn sends HR→agent the same way. Never add agent-pair-specific log helpers; fix visibility in the queue/message-activity path instead. Thread the caller `agentId` through `POST /api/internal/orchestration/staff` as `requesterId`.
