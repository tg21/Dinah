...
- **User-question handoff**. Agents that need a user decision use the orchestration `ask_user` tool (`question` + optional MCQ `options[]`; empty = free text). This sets status `awaiting-user`, persists the pending question, and emits `agent_needs_user`; a user reply clears the hold and resumes the agent. Keep the UI hold signal visible and do not route manager execution questions back through the CEO.
