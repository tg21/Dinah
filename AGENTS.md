...
- `npm run eval:integration -- --harness <name> --model <id>` runs the opt-in LLM-driven manager integration trajectory. It starts an isolated backend, invokes the selected manager model, and uses reusable fake collaborators from `tests/evals/helpers/fakeAgents.js` to advance HR, worker, and QA state between manager turns. Its result is authoritative backend-state evidence, not a model-authored event trace.
...
- **OpenCode timeout**. OpenCode invocations allow up to 120 seconds per turn because manager turns may include model latency and multiple orchestration MCP round trips; do not reduce this to the generic 30-second CLI timeout without an eval/runtime justification.
- **MCP-capable harness dispatch is asynchronous**. OpenCode must use non-blocking child-process execution because the orchestration MCP calls back into the Node backend during the harness turn; synchronous `execFileSync` deadlocks those calls. Await `spawnHarnessAgent` at routes, services, scripts, and integration tests.
...
- **Project manager invariant**. Every explicit project-creation flow (`/handleStartProject` or the CEO `create_project` orchestration tool) must call the idempotent manager guarantee and send the project brief to that project's `manager-bard`; never create a project without a manager.
